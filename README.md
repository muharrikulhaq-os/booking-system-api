...
6. **Maintenance Berkala (Odometer-based)**: Setiap kendaraan kini punya `maintenanceIntervalKm` (default 10.000 km) dan `lastMaintenanceOdometer`. Begitu `currentOdometer` naik dan selisihnya dari servis terakhir menyentuh interval (mis. baseline 80.000 km → auto-trigger di 90.000 km), sistem otomatis membuat `maintenance_records` (ditandai `isAutoGenerated: true`) dan mengubah status kendaraan ke `MAINTENANCE`. Odometer ter-update otomatis dari `POST /fuel-expenses` (field `odometerAfter`) atau `PUT /vehicles/:id`. Cek sisa km lewat `GET /vehicles/:id/maintenance-status`.
7. **Booking Type SPD / Non-SPD**: Booking kendaraan kini punya `bookingType` (`SPD` = Surat Perintah Dinas, `NON_SPD` = pemakaian umum; default `NON_SPD`).
8. **Overtime Driver (Non-SPD)**: Jam kerja normal mengikuti jadwal booking (`endDate`). Untuk booking `NON_SPD`, jika kendaraan baru selesai (`PATCH /bookings/:id/complete`) setelah `endDate`, sistem otomatis mencatat overtime supir (mis. jadwal selesai 16:30, aktual 18:00 → overtime 1 jam 30 menit). Booking `SPD` dikecualikan dari perhitungan overtime. Lihat detail di response booking (field `overtime`) atau rekap per supir di `GET /reports/driver-trips`.
...
> 🔧 **Maintenance Berkala**: Setiap kendaraan punya `maintenanceIntervalKm` (default 10.000 km) dan `lastMaintenanceOdometer` sebagai baseline. Cek endpoint `GET /:id/maintenance-status` di bawah untuk melihat sisa km sebelum maintenance otomatis di-trigger.
...
### `GET /api/v1/vehicles/:id/maintenance-status`
Cek status **maintenance berkala** (odometer-based) kendaraan — berapa km lagi sebelum maintenance otomatis di-trigger.
...
> Odometer kendaraan (`currentOdometer`) ter-update otomatis setiap kali driver input BBM/listrik dengan `odometerAfter` (`POST /fuel-expenses`) atau admin mengedit kendaraan (`PUT /vehicles/:id`). Begitu selisih `currentOdometer - lastMaintenanceOdometer` mencapai `maintenanceIntervalKm` (default 10.000 km, mis. baseline 80.000 km → trigger otomatis di 90.000 km), sistem otomatis membuat record di `GET /maintenance` (`isAutoGenerated: true`) dan mengubah status kendaraan menjadi `MAINTENANCE`. Setelah admin menyelesaikan maintenance (`PATCH /maintenance/:id/complete`), `lastMaintenanceOdometer` direset ke odometer saat itu sehingga siklus berikutnya dihitung dari sana.
...
### Jenis Booking Kendaraan (SPD / Non-SPD) & Overtime Driver
Booking kendaraan (`resource.type = VEHICLE`) punya field `bookingType`:

| Tipe | Keterangan |
|------|-----------|
| `SPD` | Surat Perintah Dinas — booking dengan surat tugas resmi. **Tidak dikenai perhitungan overtime.** |
| `NON_SPD` | Pemakaian umum/harian. Jam kerja normal mengikuti jadwal booking (`endDate`); **dikenai perhitungan overtime** bila selesai melewati jadwal. |

Field `bookingType` bersifat opsional saat `POST /bookings` (default `NON_SPD` bila tidak dikirim).

**Perhitungan overtime**: saat admin menyelesaikan booking (`PATCH /bookings/:id/complete`), jika booking bertipe `NON_SPD` dan waktu penyelesaian aktual melewati `endDate` yang dijadwalkan, sistem otomatis mencatat selisihnya sebagai overtime supir. Contoh: jadwal berakhir `16:30` tapi kendaraan baru selesai digunakan `18:00` → overtime tercatat **1 jam 30 menit**. Data ini tersimpan di tabel `driver_overtimes`, muncul di field `overtime` pada response booking (`GET /bookings/:id`, `PATCH /bookings/:id/complete`), dan direkap per supir di `GET /reports/driver-trips`.
...
```json
{
  ...
  "data": {
    ...
    "overtime": { "scheduledEndAt": "2025-06-01T16:30:00Z", "actualEndAt": "2025-06-01T18:00:00Z", "overtimeMinutes": 90 },
    "createdAt": "2025-05-26T10:00:00Z",
...
> `overtime` bernilai `null` bila booking belum `COMPLETED`, bertipe `SPD`, atau selesai tepat waktu / lebih awal.
...
| `bookingType` | string | ❌ | `SPD` \| `NON_SPD` (default `NON_SPD` jika tidak dikirim) |
...
> 🕒 **SPD vs Non-SPD & Overtime:** `SPD` (Surat Perintah Dinas) adalah booking dengan surat tugas resmi dan tidak dikenai perhitungan overtime. `NON_SPD` adalah pemakaian umum — jam kerja normal mengikuti jadwal booking (`endDate`); jika kendaraan baru diselesaikan (`PATCH /bookings/:id/complete`) setelah `endDate`, sistem otomatis mencatat overtime supir pada tabel `driver_overtimes` dan menampilkannya di field `overtime` pada response booking. Contoh: jadwal berakhir 16:30, kendaraan baru selesai 18:00 → tercatat overtime 1 jam 30 menit.

**Response `201`:** *(data booking, termasuk field `bookingType`)*
...
> ⏱️ **Overtime**: Overtime supir (untuk booking `NON_SPD` yang selesai melewati jadwal) tercatat otomatis di modul Booking (`PATCH /bookings/:id/complete`) dan direkap per supir di Modul 12 → `GET /reports/driver-trips`.
...
> ⚙️ **Maintenance Berkala (Auto-Generated)**: Selain dibuat manual, record maintenance juga bisa muncul otomatis di modul ini (`isAutoGenerated: true`) begitu odometer kendaraan mencapai kelipatan `maintenanceIntervalKm` sejak servis terakhir (lihat Modul 3 → `GET /vehicles/:id/maintenance-status`). Record auto-generated diperlakukan sama seperti manual — bisa dilihat, diupdate, dan diselesaikan lewat endpoint di bawah.
...
> ⚙️ **Maintenance Berkala (Auto-Generated)**: Selain dibuat manual lewat endpoint ini, record maintenance juga bisa dibuat otomatis oleh sistem (`isAutoGenerated: true`) begitu odometer kendaraan menyentuh `maintenanceIntervalKm` sejak servis terakhir — lihat `GET /vehicles/:id/maintenance-status`. Record auto-generated tampil di `GET /maintenance` seperti record manual dan bisa diselesaikan lewat `PATCH /maintenance/:id/complete` seperti biasa.
...
> 🧾 **Laporan SPD vs Non-SPD & Overtime**: lihat `GET /reports/driver-trips` di bagian bawah untuk rekap jumlah trip SPD vs Non-SPD dan total overtime per supir.
...
### `GET /api/v1/reports/driver-trips`
Rekap jumlah trip **SPD vs Non-SPD** per supir, beserta jumlah trip yang overtime dan total durasi overtime-nya. Hanya menghitung booking kendaraan berstatus `COMPLETED`.
...
| Parameter | Type | Keterangan |
|-----------|------|-----------|
| `startDate` | string | RFC3339, filter berdasarkan `startDate` booking |
| `endDate` | string | RFC3339, filter berdasarkan `endDate` booking |

**Response `200`:**
```json
{
  "success": true,
  "message": "Driver trips report retrieved",
  "data": [
    {
      "driverId": 1,
      "driverName": "Joko Susilo",
