...
func (s *MaintenanceService) Complete(ctx context.Context, id int32, photos []*multipart.FileHeader) (map[string]any, error) {
	...
	// Reset the odometer baseline so the next 10k-km (or whatever the
	// configured interval is) cycle counts from the current reading.
	if vehicle.ID != 0 {
		_ = s.q.UpdateVehicleLastMaintenanceOdometer(ctx, repository.UpdateVehicleLastMaintenanceOdometerParams{
			ID:                      vehicle.ID,
...
func checkAndTriggerAutoMaintenance(ctx context.Context, q repository.ExtendedQuerier, vehicleID, actorID int32) {
	...
	description := fmt.Sprintf(
		"Maintenance berkala otomatis: odometer kendaraan mencapai %d km (interval %d km sejak servis terakhir pada %d km).",
		vehicle.CurrentOdometer, vehicle.MaintenanceIntervalKm, vehicle.LastMaintenanceOdometer,
	)

	_, err = q.CreateMaintenance(ctx, repository.CreateMaintenanceParams{
		VehicleId:       vehicleID,
		Description:     description,
		Type:            "SCHEDULED",
		Status:          "ongoing",
		Odometer:        sql.NullInt32{Int32: vehicle.CurrentOdometer, Valid: true},
		Location:        "Auto-generated (odometer threshold)",
		StartDate:       time.Now(),
	...
	_, _ = q.UpdateResourceStatus(ctx, repository.UpdateResourceStatusParams{
		ID:     vehicle.ResourceId,
		Status: repository.ResourceStatusMAINTENANCE,
	})
}
